...
class FirCallCompleter(
    ...
    private fun addConstraintFromExpectedType(
        ...
        val expectedType = resolutionMode.expectedTypeRef.coneTypeSafe<ConeKotlinType>()?.fullyExpandedType(session) ?: return
    ...
    private fun Candidate.isSyntheticFunctionCallThatShouldUseEqualityConstraint(expectedType: ConeKotlinType): Boolean {
        ...
        return symbol.origin == FirDeclarationOrigin.Synthetic.FakeFunction &&
                !expectedType.isUnitOrNullableUnit &&
                !expectedType.isAnyOrNullableAny
    }

    private fun FirBasedSymbol<*>.isSyntheticElvisFunction() =
        origin == FirDeclarationOrigin.Synthetic.FakeFunction && (this as? FirCallableSymbol)?.callableId == SyntheticCallableId.ELVIS_NOT_NULL
