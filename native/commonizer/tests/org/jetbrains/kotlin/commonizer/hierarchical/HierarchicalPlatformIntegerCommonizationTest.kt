...
class HierarchicalPlatformIntegerCommonizationTest : AbstractInlineSourcesCommonizationTest() {
    ...
    fun `test platform integers in multi-target commonization`() {
        ...
        val intTarget2 = WATCHOS_ARM32.name
    ...
    fun `test platform types from known leaf targets are commonized`() {
        val result = commonize {
            outputTarget("(${LINUX_X64.name}, ${WATCHOS_ARM32.name})")
            setting(PlatformIntegerCommonizationEnabledKey, true)
            registerFakeStdlibIntegersDependency("(${LINUX_X64.name}, ${WATCHOS_ARM32.name})")
        ...
        result.assertCommonized(
            "(${LINUX_X64.name}, ${WATCHOS_ARM32.name})", """
    ...
    fun `test platform types from unknown targets are not commonized`() {
        val result = commonize {
            outputTarget("(unknown, ${WATCHOS_ARM32.name})")
            setting(PlatformIntegerCommonizationEnabledKey, true)
            registerFakeStdlibIntegersDependency("(unknown, ${WATCHOS_ARM32.name})")
        ...
        result.assertCommonized(
            "(unknown, ${WATCHOS_ARM32.name})", "".trimIndent()
        )
    }
    ...
    fun `test multiple argument function with over the edge type alias available`() {
        val result = commonize {
            outputTarget("(${WATCHOS_ARM32.name}, ${IOS_ARM64.name})")
            setting(PlatformIntegerCommonizationEnabledKey, true)
            registerFakeStdlibIntegersDependency("(${WATCHOS_ARM32.name}, ${IOS_ARM64.name})")
        ...
        result.assertCommonized(
            "(${WATCHOS_ARM32.name}, ${IOS_ARM64.name})", """
