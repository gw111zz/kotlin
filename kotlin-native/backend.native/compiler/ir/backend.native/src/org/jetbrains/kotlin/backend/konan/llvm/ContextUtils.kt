...
internal interface ContextUtils : RuntimeAware {
    ...
    fun IrFunction.returnsObjectType(): Boolean =
            returnType.binaryTypeIsReference() && getLlvmFunctionReturnType(this).llvmType != llvm.voidType
    ...
    val runtimeAnnotationMap by lazy {
        StaticData.getGlobal(module, "llvm.global.annotations")
                ...
                ?.groupBy(
                        {
                            LLVMGetInitializer(
                                    if (bitcodeContext.config.useOpaquePointers) LLVMGetOperand(it, 1)
                                    else LLVMGetOperand(LLVMGetOperand(it, 1), 0)
                            )?.getAsCString() ?: ""
                        },
                        {
                            if (bitcodeContext.config.useOpaquePointers) LLVMGetOperand(it, 0)!!
...
internal class CodegenLlvmHelpers(private val generationState: NativeGenerationState, module: LLVMModuleRef) : BasicLlvmHelpers(generationState, module), RuntimeAware {
    ...
    private fun importMemset(): LlvmCallable {
        ...
        return llvmIntrinsic(
                if (context.config.useOpaquePointers) "llvm.memset.p0.i32"
    ...
    internal fun externalFunction(llvmFunctionProto: LlvmFunctionProto): LlvmCallable {
        ...
        if (found != null) {
            ...
            return LlvmCallable(functionType, llvmFunctionProto.signature.returnsObjectType, found, llvmFunctionProto.signature)
        } else {
    ...
    internal fun externalNativeRuntimeFunction(
            ...
            LlvmFunctionSignature(returnType, false, parameterTypes, isVararg, functionAttributes).toProto(
                    name,
