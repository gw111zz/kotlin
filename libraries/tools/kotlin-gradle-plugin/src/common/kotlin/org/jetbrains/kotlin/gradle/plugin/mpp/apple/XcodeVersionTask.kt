...
internal val XcodeVersionSetupAction = KotlinProjectSetupAction {
    launch {
        ...
        tasks.withType<UsesXcodeVersion>().configureEach { task ->
            task.xcodeVersionFile.set(xcodeVersionTask.flatMap { it.outputFile })
        }
    }
}
...
internal interface UsesXcodeVersion : Task {
    ...
    @get:Optional
    @get:InputFile
    val xcodeVersionFile: RegularFileProperty

    @get:Internal
    val xcodeVersion: XcodeVersion
        get() {
            val version = xcodeVersionFile.getFile().readText()
            return XcodeVersion.parse(version) ?: error("Couldn't parse Xcode version from '$version'")
        }
}
