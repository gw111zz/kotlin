...
// Must be in sync with TestVersions.kt
val gradleVersions =
    listOf("6.8.3", "6.9.4", "7.0.2", "7.1.1", "7.2", "7.3.3", "7.4.2", "7.5.1", "7.6.3", "8.0.2", "8.1.1", "8.2.1", "8.3", "8.4", "8.5")
...
junitTags.forEach { junitTag ->
    ...
    val tasksByGradleVersion = gradleVersions.map { gradleVersion ->
        tasks.register<Test>("${taskPrefix}TestsForGradle_${gradleVersion.replace(".", "_")}") {
            ...
            description = "Runs all tests for Kotlin Gradle plugins"
    ...
    tasks.register("${taskPrefix}TestsByGradleVersion") {
        group = gradleVersionTaskGroup
